...
void main() {
  ...
  final tModel = Stage1FormModel(
    ...
    name: 'Juan Pérez',
    gaveras: [
      {'quantity': 5, 'referenceWeight': 12.5, 'gaveraType': 'Kilo'},
    ],
    baskets: [
      {'size': 'grande', 'quantity': 3},
    ],
  ...
  group('Stage1FormModel', () {
    test('fromJson construye el modelo correctamente', () {
      ...
      expect(result.baskets.first['quantity'], 3);
      expect(result.gaveras.first['quantity'], 5);
      expect(result.gaveras.first['gaveraType'], 'Kilo');
    ...
    test('toEntity convierte al entity de dominio correctamente', () {
      ...
      expect(entity.gaveras.first.gaveraType, 'Kilo');
      expect(entity.baskets.first.size, BasketSize.grande);
      expect(entity.baskets.first.quantity, 3);
    });
    ...
    test('fromEntity construye el modelo desde el entity', () {
      final entity = Stage1FormData(
        ...
        name: 'Juan Pérez',
        gaveras: [
          GaveraData(quantity: 5, referenceWeight: 12.5, gaveraType: 'Kilo'),
        ],
        baskets: [BasketData(size: BasketSize.grande, quantity: 3)],
        preservativesWeight: 1.5,
        preservativesJars: 2,
        limeWeight: 0.8,
        limeJars: 1,
      ...
      expect(model.gaveras.first['gaveraType'], 'Kilo');
      expect(model.baskets.first['size'], 'grande');
      expect(model.baskets.first['quantity'], 3);
    });
